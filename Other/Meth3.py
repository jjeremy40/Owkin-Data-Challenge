...
"""CNN: LeNet-5
Train : sum of masks on the axis=2 then normalized for patient Public dataset (features 92x92, 199 patients) and Private dataset (features 92x92, 101 patients)
...
result C-index : train public : 0.448 on public and 0.503 on private
                 train private : 0.476 on public and 0.535 on private
"""
import numpy as np
import pandas as pd
...
id_patient_train_public = (train_buff_clinical.loc[train_buff_clinical['SourceDataset']=='l1',:].reset_index().values)[:,0].astype(int)
id_patient_train_private = (train_buff_clinical.loc[train_buff_clinical['SourceDataset']=='l2',:].reset_index().values)[:,0].astype(int)

basepath = 'x_train/images/patient_'
train_base_public = np.empty((199, 92, 92))
train_base_private = np.empty((101, 92, 92))

for i in range(199):
    if (id_patient_train_public[i]-10) < 0:
        scan = np.load(basepath + '00' + str(id_patient_train_public[i]) + '.npz')['mask']
        train_base_public[i,:,:] = scan.sum(axis=2)/92
...
for i in range(101):
    if (id_patient_train_private[i]-10) < 0:
        scan = np.load(basepath + '00' + str(id_patient_train_private[i]) + '.npz')['mask']
        train_base_private[i,:,:] = scan.sum(axis=2)/92
    elif (id_patient_train_private[i]-100) < 0:
        scan = np.load(basepath + '0' + str(id_patient_train_private[i]) + '.npz')['mask']
        train_base_private[i,:,:] = scan.sum(axis=2)/92
    else :
        scan = np.load(basepath + str(id_patient_train_private[i]) + '.npz')['mask']
        train_base_private[i,:,:] = scan.sum(axis=2)/92

train_buff_output = pd.read_csv('x_train/output.csv', index_col=0)
train_label_public = (train_buff_output.loc[train_buff_clinical['SourceDataset']=='l1',:].reset_index().values)[:,1].astype(float)
train_label_private = (train_buff_output.loc[train_buff_clinical['SourceDataset']=='l2',:].reset_index().values)[:,1].astype(float)

del train_buff_output, basepath, i, scan, id_patient_train_public, id_patient_train_private, train_buff_clinical
...
#for CNN :
train_base_public = train_base_public.reshape(len(train_base_public),train_base_public.shape[1], train_base_public.shape[2], 1)
train_base_private = train_base_private.reshape(len(train_base_private),train_base_private.shape[1], train_base_private.shape[2], 1)
...
model.add(tkl.ZeroPadding2D(2, input_shape=(92, 92, 1)))
...
#history = model.fit(train_base_private, train_label_private, batch_size=30, epochs=100)
